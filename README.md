...
- **Tracked mitigation measures** (`backend/main.py`, `backend/db.py`): a stable, persisted set of 3 AI-suggested measures per city (structured JSON, not the freeform narrative text — that reworded itself every load, so status could never mean anything across visits), generated once and kept in the `mitigation_measures` table. Each has a Proposed → In Progress → Implemented status (timestamped on every change), an optional note, and — once Implemented with a start date — an impact chart comparing its target metric before/after that date.
- **ESG score** (`backend/esg_score.py`): a transparent 0-100 composite built only from data already in the app — Environmental (0-70) from the share of historical readings with a "Good" composite AQI, Governance (0-30) from mitigation-measure completion rate. The exact formula is returned as `methodology` in the API response and shown in the UI.
- **Export Report**: client-side PDF and CSV export (current metrics, AI analysis summary, ESG score, mitigation status) — no server round-trip.
- **Language toggle**: English/Sinhala/Tamil on section headers, metric names, tab names, and the status legend (AI-generated translations — see the caveat below).
- **Dark mode**: follows the OS preference by default, toggle persists to `localStorage`.
...
## Mitigation tracking & ESG score

No setup needed beyond the database from [Historical trends setup](#historical-trends-setup) — the `mitigation_measures` table is created automatically alongside `readings`. The first time you open the Mitigation Measures tab for a city, it generates 3 measures via the AI and stores them; after that they're stable (edit status/notes instead of them being regenerated).

The ESG score methodology is intentionally simple and disclosed in full in the UI (under the score itself) and in `backend/esg_score.py`'s `METHODOLOGY` constant — it is **not** aligned to GRI, CDP, or any other external ESG framework. Treat it as a directional indicator built from this app's own data, not a certified disclosure metric.
